...
def test_should_clear_hashtable(hash_table):
    empty_hashtable = hash_table.clear()
    assert empty_hashtable == None

def test_should_update_hastable_with_dict(hash_table):
    dict = {98.6: 98.6, "hola": "hola", False: False}
    hash_table.update(dict)

    assert hash_table[98.6] == 98.6
    assert hash_table["hola"] == "hola"
    assert  hash_table[False] == False

def test_should_update_hastable_with_list(hash_table):
    list = [(98.6, 98.6), ("hola", "hola"), (False, False)]
    hash_table.update(list)

    assert hash_table[98.6] == 98.6
    assert hash_table["hola"] == "hola"
    assert hash_table[False] == False

def test_should_update_hastable_with_keyword_arguments(hash_table):
    hash_table.update(altura=1.80, ciao='ola')
